...
async def word_match(word: str, file_path: Path):
    ...
    """
    # 去除file_path中的前缀路径
    output_path = file_path.relative_to(config.CORPUS_PATH)
    # 获得一系列匹配的文本片段
    matched: list[str] = []
    try:
...
def sub_corpus_match(word: str, sub_path: str):
    ...
    async def process_files():
        ...
        for batch in [files[i:i + config.BATCH_SIZE] for i in range(0, len(files), config.BATCH_SIZE)]:
            batch_tasks = [word_match(word, file_path) for file_path in batch]
